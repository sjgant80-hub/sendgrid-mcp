#!/usr/bin/env node
// sendgrid-mcp · stdio MCP server exposing foldkit as Claude-callable tools + resources
...
const server = new Server(
  { name: 'sendgrid-mcp', version: '1.0.0' },
...
server.setRequestHandler(CallToolRequestSchema, async (req) => {
  try {
    ...
    return {
      isError: true,
      content: [{ type: 'text', text: `sendgrid-mcp error: ${err.message}` }]
    };
  }
});
